...
void problem(DomainS *pDomain)
{
  ...
  */
  ...
  xRdroplet = Rdroplet*1.0*pGrid->dx1;

  //slow down hot gas inside droplets
  shear = par_getd_def("problem","shear",0);
  
  ndroplets = (int) (fv * SQR(r_cloud/xRdroplet));
...
/*   } */
  ...
  for (l=0; l<ndroplets; l++) {
    tmp1 = sqrt(RandomReal()) * (r_cloud-dr-xRdroplet);
    tmp2 = 2.0 * PI *RandomReal();
    xd[l] = tmp1*cos(tmp2);
    yd[l] = tmp1*sin(tmp2);
  }
...
/* #endif        */
  ...
  for (k=ks; k<=ke; k++) {
	...
	for (l = 0; l < ndroplets; l++){
	  if(SQR(x1-xd[l])+SQR(x2-yd[l])<SQR(xRdroplet)){
	    rho = drat;
	    /* if(l==2263){ */
