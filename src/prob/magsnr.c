...
static Real drat, vflow, vflow0, betain, betaout,dr,bz, prat, fudge_cool;
...
void problem(DomainS *pDomain)
{
  ...
  prat   = par_getd_def("problem", "prat", 10.0); // overpressure
  fudge_cool = par_getd_def("problem","fudge_cool", 1.0); //density for cooling function
  ...
  */
  ...
  retemp = par_getd_def("problem","retemp",1.0);
  ...
  for (k=ks; k<=ke; k++) {
    for (j=js; j<=je; j++) {
      for (i=is; i<=ie; i++) {
        ...
        if (dr > 0.0){
          rho = 1.0; //(1.0 + drat*0.5*(1.0+tanh((r_cloud-r)/(dr*r_cloud))));
  ...
  for (k=ks; k<=ku; k++) { //fixed ke+1 bug?
    for (j=js; j<=je; j++) {
      for (i=is; i<=ie; i++) {
        pGrid->B3i[k][j][i] = 0.0;
        if(tangled){
...
void problem_read_restart(MeshS *pM, FILE *fp)
{
  ...
  drat  = par_getd("problem", "drat");
  fudge_cool = par_getd_def("problem", "fudge_cool", 1.0);
  vflow = par_getd("problem", "vflow");
  vflow0 = vflow;
  ...
  for (k=ks; k<=ke; k++) {
    for (j=js; j<=je; j++) {
      for (i=is; i<=ie; i++) {
        ...
        /* find temp in keV */
        temp = W.P/W.d;
        temp = newtemp_townsend(W.d*fudge_cool, temp, pG->dt);
