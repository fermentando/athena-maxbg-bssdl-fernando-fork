...
/* unit vectors */
Real3Vect get_B01(Real r, Real theta, Real phi);
...
void problem(DomainS *pDomain)
{
  ...
  */
  ...
  if(tangled){
    ...
    nterms = par_getd_def("problem", "nterms",1);
    nterms = 1; // only doing B01
    alpha = par_getd_def("problem","alpha",50.0);
    // alpha is now determined by radius of cloud and setting to zero for BR
    alpha = r_cloud/4.49340945790906421;
    for (i=0; i<nterms; i++) {
      if (myID_Comm_world == 0) {
        ...
        amp   = 1./35.0; // RandomNormal2(1.0, 0.25);
...
static Real cooling_func(const Real rho, const Real P, const Real dt)
{
  ...
  Edot = (temp < tcloud) ? 0.0 : 1000.0 * exp(-2.0 * temp / tcloud);
  ...
  for (k=0; k<=ke+nghost; k++) {
    for (j=0; j<=je+nghost; j++) {
      for (i=0; i<=ie+nghost; i++) {
        ...
        r.x2 -= 0.5 * pG->dx2;
        r.x3 -= 0.5 * pG->dx3;
	rho = sqrt(SQR(r.x1)+SQR(r.x2)+SQR(r.x3))/alpha;
	theta = acos(r.x3/rho/alpha);
	if(r.x3/rho/alpha > 1.0){
	  theta = acos(1.0);
	}
	...
	e2 = get_e2(theta, phi);
	e3 = get_e3(theta, phi);
	B01 = get_B01(rho,theta,phi);
	
        
	if(rho > 1.e-10){ //bigger than zero
	  
	  A[k][j][i].x1 += amp * (e1.x1 * B01.x1 + e1.x2 * B01.x2 + e1.x3 * B01.x3);
	  A[k][j][i].x2 += amp * (e2.x1 * B01.x1 + e2.x2 * B01.x2 + e2.x3 * B01.x3);
	  A[k][j][i].x3 += amp * (e3.x1 * B01.x1 + e3.x2 * B01.x2 + e3.x3 * B01.x3);
	}
	else{
	  A[k][j][i].x3 += amp * 2.0/3.0;
	}
	if(B01.x1 != B01.x1 || B01.x2 != B01.x2 || B01.x3 !=B01.x3 ){
	  printf("Nanned B field!\n");
	  printf("Bs %f %f %f  xyz %f %f %f  rho theta phie %f %f %f\n",B01.x1, B01.x2, B01.x3, r.x1, r.x2, r.x3, rho, theta, phi);
...
{
  Real3Vect ret;
  ret.x1 = 2.0*cos(theta)*(sin(r)/SQR(r)/r - cos(r)/SQR(r));
  ret.x2 = -1.0*sin(theta)*(sin(r)/r*(1.0-1.0/SQR(r))+cos(r)/SQR(r));
  ret.x3 = sin(theta)*(sin(r)/SQR(r)-cos(r)/r);
...
{
  Real3Vect ret;
  ret.x1 = sin(theta) * sin(phi);
  ret.x2 = cos(theta) * sin(phi);
  ret.x3 = cos(phi);
