...
static void bc_shifted_periodic_ix2(GridS *pGrid);
static void bc_shifted_periodic_ox2(GridS *pGrid);
...
void problem(DomainS *pDomain)
{
  ...
  ishift = par_geti("domain1", "ishift");
  kshift = par_geti("domain1", "kshift");

  Real Bx2 = par_getd("domain1", "x2max") - par_getd("domain1", "x2min");
  Real dx2 = Bx2 / par_getd("domain1", "Nx2");

  Real Lcold = sqrt(Bx2*Bx2 + SQR(ishift * dx2));
  ...
  // enroll custom bcs
  if (pDomain->Disp[1] == 0)
    bvals_mhd_fun(pDomain, left_x2,  bc_shifted_periodic_ix2);
  if (pDomain->MaxX[1] == pDomain->RootMaxX[1])
    bvals_mhd_fun(pDomain, right_x2, bc_shifted_periodic_ox2);
  ...
  for (k=ks; k<=ke; k++) {
    for (j=js; j<=je; j++) {
      for (i=is; i<=ie; i++) {
        ...
        Real dr = 0.025 * Lcold;
        Real xshift = ishift * dx2 / Bx2 * y;
        Real myx = fabs(x + xshift) - Lcold; // < 0 for cold material
        Real myy = sqrt(y*y + xshift * xshift);

        Real x_boundary = bound_amp * sin(PI*myy / bound_lambda); //-5.0+5.0*sin(0.1*PI*y)
        Real dens = (rho_c-rho_h)* 1.0 * (0.5 - atan((myx-x_boundary)/dr)/PI) + rho_h;
...
static int _shift_index(int i, int is, int ie, int ish) {
  return (i - is - nghost + ish) % (ie + nghost + 1) + is;
}
  ...
  for (k=ks; k<=ke; k++) {
    for (j=1; j<=nghost; j++) {
      for (i=is-nghost; i<=ie+nghost; i++) {
        ci = _shift_index(i, is, ie, ishift);
        //ci = i;
        pGrid->U[k][js-j][i] = pGrid->U[k][je-(j-1)][ci];
      }
    }
  }
  ...
  for (k=ks; k<=ke; k++) {
    for (i=is; i<=ie; i++) {
      for (j=1; j<=nghost; j++) {
        ...
        ci = _shift_index(i, is, ie, ishift);
        pGrid->U[k][js-j][i] = pGrid->U[ck][je-(j-1)][ci];
        //printf("[ix2] (%d,%d,%d) = (%d,%d,%d)\n", k, js-j, i, ck, je-(j-1), ci);
      }
    }
  }
  */
  ...
  for (k=ks; k<=ke; k++) {
    for (j=1; j<=nghost; j++) {
      for (i=is-nghost; i<=ie+nghost; i++) {
        ci = _shift_index(i, is, ie, ishift);
        //ci = i;
        pGrid->U[k][je+j][ci] = pGrid->U[k][js+(j-1)][i];
      }
    }
  }
  ...
  for (k=ks; k<=ke; k++) {
    for (i=is; i<=ie; i++) {
      for (j=1; j<=nghost; j++) {
        ck = _shift_index(k, ks, ke, kshift);
        ci = _shift_index(i, is, ie, ishift);
        pGrid->U[ck][je+j][ci] = pGrid->U[k][js+(j-1)][i];
        //printf("[ox2] (%d,%d,%d) = (%d,%d,%d)\n", ck, je+j, ci, k, js+(j-1), i);
      }
    }
  }
  */
