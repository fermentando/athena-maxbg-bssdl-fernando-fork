    ...
    for (j=js; j<=je; j++) {
      for (i=is; i<=ie; i++) {
        ...
        if (dr > 0.0){
          ...
          //commented out for advection test
  ...
  /* seed a perturbation */
   for (k=ks; k<=ke; k++) {
    for (j=js; j<=je; j++) {
	  ...
	  // change energy (pressure) instead of density
	  pGrid->U[k][j][i].E += (1.0+fact);
...
/*==============================================================================
 * BOUNDARY CONDITIONS:  Removed for advection test. Should be periodic
 *
 *----------------------------------------------------------------------------*/
